...
const vec3 levels3d = vec3(levels,levels,levels);
...
int dpat(vec2 c) {
	int x = int(floor(c.x)) & 1;
	int y = int(floor(c.y)) & 1;
	return (y * 2) + x;
}

vec3 dpat3d(vec2 c) {
	float v = float(dpat(c)) / 4.0;
	return vec3(v,v,v);
}
...
void main() {
	...
	vec3 ditherAdd = dpat3d(vec2(gl_FragCoord.x,gl_FragCoord.y));
	vec3 bColor = floor((finalColour * levels3d) + ditherAdd) / levels3d;
	fragColour = vec4(bColor, colour.w);	
}
